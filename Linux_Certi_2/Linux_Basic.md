...
+ ```fdisk```
+ 📌 ```blkid```: 파티션에 할당된 UUID 값을 확인하는 명령어
### LVM(Logic Volume Manager)
+ 여러 개의 물리적인 하드디스크를 논리적인 디스크로 할당하여 유연하게 관리할 수 있도록 도와준다.
+ 서버 운영시 대용량 저장공간이 필요할 때 주로 사용
+ ⭐️Physical Volume -> Volume Group -> Logical Volume
  + 📌 PE: 파일 시스템을 구성하는 일반적인 디스크의 블록에 해당하고 일정한 크기를 갖는다.
+ ⭐️RAID(Redundant Array of Independent Disks)
+ Physical Volume -> Volume Group -> Logical Volume
  + 📌 논리적 볼륨(LV): 사용자가 필요한만큼 할당하여 만들어지는 공간으로 물리적 디스크에서 분할하여 생성하는 파티션과 같은 개념이다.
#### RAID(Redundant Array of Independent Disks)
  ...
  + 레벨에 따라 데이터를 저장하는 방법이 상이
    ...
    + RAID 0: ```스트라이핑```을 이용, 논리적으로 연속된 여러 개의 물리적인 디스크 드라이브에 나누어 저장하는 기술, 📌 디스크 오류 대처와 가장 거리가 멀다.
...
+ 디렉터리 및 파일
  + 디렉터리 관련 명령어
    ...
    unalias ls      // ls 명령어에 설정된 에일리어스(alias)를 해제하는 명령
    ```
  + 파일 관련 명령어
    ```shell
    Is
  ...
  + 파일 내용 출력 명령어
    ...
    split           // 여러 개의 파일로 분할
    ```
    + ```cat ~ihduser/.bash_history```: root 권한으로 ihduser 사용자가 실행한 명령의 목록 정보를 확인하는 과정.
    + 📌 ```grep ihduser /etc/passwd```:  ihduser 사용자의 로그인 셸을 확인하는 명령
  + 파일 내용 비교 명령어
    ```shell
    diff 
